...
const SearchResults = () => {
  ...
  const convertBase64ToDataURL = (base64String, mimeType = 'image/jpeg') => {
    if (!base64String) return "https://placehold.co/400x400?text=No+Image"; 
            ...
            {searchData.map((product) => (
              <div key={product.id} className="col">
                <div className="card h-100 shadow-sm">
                  <img 
                    // FIX: Changed productImage to imageData and passed the correct imageType
                    src={convertBase64ToDataURL(product.imageData, product.imageType)} 
                    className="card-img-top p-3" 
                    alt={product.name}
                    style={{ height: "200px", objectFit: "contain", cursor: "pointer" }}
                    onClick={() => handleViewProduct(product.id)}
                    // Added a fallback just in case the base64 string is corrupted
                  ...
                  <div className="card-body d-flex flex-column">
                    ...
                    <h5 className="card-text text-primary mt-auto mb-3">₹{product.price ? product.price.toLocaleString('en-IN') : '0'}</h5>
